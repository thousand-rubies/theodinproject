...
RSpec.describe TailwindFormBuilder do
  ...
  describe '#text_field' do
    ...
    context 'when the options contains classes' do
      it 'append the class to the text field' do
        expected_html = <<~HTML
          <div class="mt-1 relative rounded-md shadow-sm">
          <input class="block w-full border rounded-md py-2 px-3 focus:outline-none sm:text-base border-gray-300 focus:ring-indigo-500 focus:border-indigo-500 test-class" type="text" name="user[username]" id="user_username" />
          </div>
        HTML
  ...
  describe '#check_box' do
    it 'returns a tailwind styled check box' do
      expected_html = <<~HTML
        <input name="user[username]" type="hidden" value="0" /><input class="h-4 w-4 border-gray-300 rounded " type="checkbox" value="1" name="user[username]" id="user_username" />
      HTML
